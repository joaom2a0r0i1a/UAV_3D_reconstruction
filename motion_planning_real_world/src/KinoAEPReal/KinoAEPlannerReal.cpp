...
KinoAEPlanner::KinoAEPlanner(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private) : nh_(nh), nh_private_(nh_private), segment_evaluator(nh_private_), voxblox_server_(nh_, nh_private_) {
    ...
    sub_local_pose_diag = mrs_lib::SubscribeHandler<geometry_msgs::PoseStamped>(shopts, "local_pose_in", &KinoAEPlanner::callbackLocalPose, this);
    sub_local_velocity_diag = mrs_lib::SubscribeHandler<geometry_msgs::TwistStamped>(shopts, "local_velocity_in", &KinoAEPlanner::callbackLocalVelocity, this);
    ...
    ss_offset = nh_private_.advertiseService("offset_in", &KinoAEPlanner::callbackOffset, this);
...
void KinoAEPlanner::AEP() {
    ...
    if (goto_global_planning) {
        ...
        if (go_terminate) {
            ROS_INFO("[KinoAEPlanner]: No information gain. Terminate.");
            changeState(STATE_STOPPED);
            return;   
        }
...
bool KinoAEPlanner::callbackOffset(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res) {
    if (!is_initialized) {
        res.success = false;
        res.message = "not initialized";
        return true;
...
void KinoAEPlanner::callbackLocalPose(const geometry_msgs::PoseStamped::ConstPtr msg) {
    if (!is_initialized) {
        return;
    }
    ROS_INFO_ONCE("[KinoAEPlanner]: getting LocalPose diagnostics");
    ...
    if (std::isnan(q.x) || std::isnan(q.y) || std::isnan(q.z) || std::isnan(q.w)) {
        ROS_ERROR("[KinoAEPlanner]: Invalid quaternion received (contains NaNs)");
        return;
    }
    
    double norm = std::sqrt(q.x * q.x + q.y * q.y + q.z * q.z + q.w * q.w);    
    if (norm < 0.1 || norm > 1.1) {
      ROS_WARN_THROTTLE(5, "[KinoAEPlanner] Invalid quaternion detected. Norm: %.3f. Skipping this pose.", norm);
      return;
    }
    ...
    try {
        ...
        ROS_ERROR_THROTTLE(1.0, "[KinoAEPlanner]: Exception during getYaw(): %s — skipping this pose.", e.what());
        return;
    }
...
void KinoAEPlanner::callbackLocalVelocity(const geometry_msgs::TwistStamped::ConstPtr msg) {
    ...
    ROS_INFO_ONCE("[KinoAEPlanner]: getting LocalVelocity diagnostics");
