...
void AEPlanner::localPlanner() {
    ...
    std::pair<double, double> result_root = segment_evaluator.computeGainOptimizedAEP(trajectory_point, initial_offset);
    ...
    while (j < N_max || best_score_ <= g_zero) {
        ...
        for (size_t i = 1; i < best_branch.size(); ++i) {
            ...
            std::pair<double, double> result_bb = segment_evaluator.computeGainOptimizedAEP(trajectory_point, initial_offset);
