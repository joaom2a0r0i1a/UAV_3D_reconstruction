#ifndef VOXBLOX_PLANNING_GAIN_EVALUATOR_2D_H_
#define VOXBLOX_PLANNING_GAIN_EVALUATOR_2D_H_
...
class GainEvaluator2D {
 public:
  ...
  void computeGainFromsampledYaw(const std::shared_ptr<rrt_star_2d::Node>& node, int yaw_samples, eth_mav_msgs::EigenTrajectoryPoint& trajectory_point);

  void computeCost(std::shared_ptr<rrt_star_2d::Node>& new_node);

  void computeScore(std::shared_ptr<rrt_star_2d::Node>& new_node, double lambda);
...
#endif  // VOXBLOX_PLANNING_COMMON_GAIN_EVALUATOR_2D_H_