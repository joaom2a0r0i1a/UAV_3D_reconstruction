...
class EvalPlotting(object):
    ...
    def __init__(self):
        ...
        if self.multi_series:
            self.evaluate_multi_series(target_dir)
        elif self.series:
    ...
    def evaluate_multi_series(self, target_dir):
        ...
        #series_dir = ["AEP", "AEP Prior"]
        #series_dir = ["AEP", "AEP Prior", "KAEP", "KAEP Prior"]
        #series_dir = ["AEP_school_cost", "AEP_school_gain", "AEP_school_score"]
        #series_dir = ["KRH-NBVP (ours)", "KRH-NBVP (ours) prior", "KAEP (ours)", "KAEP (ours) prior"]
        ...
        for idx, series in enumerate(series_dir):
            ...
            axes[1, 0].plot(x, means['StdDevError'], color=colors[idx], label=series)
            ...
            if np.max(means['UnknownVoxels']) > 0:
                ...
                axes[0, 1].fill_between(x,
                                        unknown - 100 * std_devs['UnknownVoxels'],
                                        unknown + 100 * std_devs['UnknownVoxels'],
                                        facecolor=colors[idx],
                                        alpha=.2)
