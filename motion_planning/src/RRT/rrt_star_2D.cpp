#include "motion_planning_python/RRT/rrt_star_2D.h"
...
Eigen::Vector3d sampleSpace(double dim_x, double dim_y) {
    ...
    return Eigen::Vector3d(rand_x, rand_y, 0);
}
...
void computeSamplingDimensions(double radius, Eigen::Vector3d& result) {
    ...
    while (!solutionFound) {
        rand_x = dis(gen);
        rand_y = dis(gen);
        if (Eigen::Vector3d(rand_x, rand_y, 0).norm() > radius) {
            continue;
        }
        solutionFound = true;
    }

    result = Eigen::Vector3d(rand_x, rand_y, 0);
}
...
void findNearest(const std::vector<std::shared_ptr<Node>>& tree, const Eigen::Vector3d& point, std::shared_ptr<Node>& nearestNode) {
    ...
    for (const auto& node : tree) {
        double distance = (node->point.head<2>() - point.head<2>()).norm();
        if (distance < minDist) {
            minDist = distance;
            nearestNode = node;
        }
...
void steer(const std::shared_ptr<Node>& fromNode, const Eigen::Vector3d& toPoint, double stepSize, std::shared_ptr<Node>& result) {
    double dist = (toPoint.head<2>() - fromNode->point.head<2>()).norm();
    if (dist < stepSize) {
        result = std::make_shared<Node>(Eigen::Vector4d(toPoint.x(), toPoint.y(), toPoint.z(), fromNode->point.w()));
    } else {
        Eigen::Vector2d direction = (toPoint.head<2>() - fromNode->point.head<2>()).normalized();
        Eigen::Vector2d newPoint = fromNode->point.head<2>() + stepSize * direction;
        result = std::make_shared<Node>(Eigen::Vector4d(newPoint.x(), newPoint.y(), toPoint.z(), fromNode->point.w()));
    }
}
...
void steer_parent(const std::shared_ptr<Node>& fromNode, const Eigen::Vector3d& toPoint, double stepSize, std::shared_ptr<Node>& new_node) {
    ...
    if (dist < stepSize) {
        ...
        Eigen::Vector2d direction = (toPoint.head<2>() - fromNode->point.head<2>()).normalized();
        Eigen::Vector2d newPoint = fromNode->point.head<2>() + stepSize * direction;
        new_node = std::make_shared<Node>(Eigen::Vector4d(newPoint.x(), newPoint.y(), toPoint.z(), fromNode->point.w()));
        new_node->parent = fromNode;
    }
}
...
void chooseParent(std::shared_ptr<Node>& new_node, const std::vector<std::shared_ptr<Node>>& nearbyNodes) {
    ...
    for (const auto& node : nearbyNodes) {
        double cost = node->cost + (node->point.head<2>() - new_node->point.head<2>()).norm();
        if (cost < minCost) {
            minCost = cost;
            parent = node;
        }
...
bool rrtStar(const Eigen::Vector4d& start, const Eigen::Vector4d& goal,
    ...
    for (int i = 0; i < max_iter; ++i) {
        ...
        if (!collides(newNode->point.head<2>(), obstacles)) {
            ...
            if ((newNode->point.head<2>() - goal.head<2>()).norm() <= tolerance) {
                ...
                for (size_t i = 0; i < path.size() - 1; ++i) {
                    path[i][2] = calculateYawAngle(std::make_shared<Node>(path[i]), std::make_shared<Node>(path[i + 1]));
                    if (i == path.size() - 2) {
                        path[i + 1][3] = goal[3];
                    }
                }
