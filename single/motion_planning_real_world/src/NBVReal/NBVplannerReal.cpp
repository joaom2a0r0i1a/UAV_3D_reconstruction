...
void NBVPlanner::explorationSweep() {
    ...
    if (z_top <= start[2] + 0.05) {
        ROS_WARN("[NBVPlanner]: Exploration sweep skipped: no headroom (z %.2f, ceiling %.2f).",
                 start[2], ceiling);
        return;
    }
    ROS_INFO("[NBVPlanner]: Exploration sweep: up %.2f -> %.2f m, rotate, down.", start[2], z_top);
    ...
    ROS_INFO("[NBVPlanner]: Exploration sweep done.");
}
