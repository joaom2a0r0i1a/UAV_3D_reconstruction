...
class EvalData(object):
    def __init__(self):
        ...
        self.ns_planners = [rospy.get_param(f'~ns_planner_uav_{i+1}', f"/uav{i+1}/planner_node") for i in range(self.num_uavs)]
        ...
        self.reset_ros = rospy.get_param('~reset_ros', True)  # On shutdown stops the planner
        ...
        if self.evaluate:
            ...
            for i in range(self.num_uavs):
                rospy.set_param(self.ns_planners[i] + "/performance_log_dir", self.eval_directory)
            ...
            for i in range(self.num_uavs):
                self.points_sub = rospy.Subscriber(f"points_{i+1}", PointCloud2,
                                            self.points_callback, queue_size=10)
                self.pointcloud_subs.append(self.points_sub)
    ...
    def launch_simulation(self):
        ...
        if self.startup_timeout > 0.0:
            try:
                for i in range(self.num_uavs):
                    rospy.wait_for_message(f"/uav{i+1}/" + "simulation_ready", Bool,
        ...
        if self.startup_timeout > 0.0:
            try:
                for ns_planner in self.ns_planners:
                    rospy.wait_for_service(ns_planner + "/start",
                                        self.startup_timeout)
            except rospy.ROSException:
        ...
        for ns_planner in self.ns_planners:
            run_planner_srv = rospy.ServiceProxy(ns_planner + "/start", Trigger)
            run_planner_srv()
    ...
    def stop_experiment(self, reason):
        ...
        if self.reset_ros:
            try:
                # Stops the planners
                for ns_planner in self.ns_planners:
                    terminate_srv = rospy.ServiceProxy(ns_planner + "/stop", Trigger)
                    terminate_srv()
            except:
                pass
        width = len(reason) + 4
