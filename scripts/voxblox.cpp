...
voxblox::TsdfServer& VoxbloxMap::getTSDFServer() { return *tsdf_server_; }

void VoxbloxMap::setupFromParamMap(Module::ParamMap* param_map) {
  // create an tsdf server
  ros::NodeHandle nh("");
  ros::NodeHandle nh_private("~");
  tsdf_server_.reset(new voxblox::TsdfServer(nh, nh_private));
  tsdf_server_->setTraversabilityRadius(
      planner_.getSystemConstraints().collision_radius);

  // cache constants
  c_voxel_size_ = tsdf_server_->getTsdfMapPtr()->voxel_size();
  c_block_size_ = tsdf_server_->getTsdfMapPtr()->block_size();
...
bool VoxbloxMap::isTraversable(const Eigen::Vector3d& position,
  ...
  double distance = 0.0;
  if (tsdf_server_->getTsdfMapPtr()->getDistanceAtPosition(position,
                                                           &distance)) {
...
bool VoxbloxMap::isObserved(const Eigen::Vector3d& point) {
  return tsdf_server_->getTsdfMapPtr()->isObserved(point);
}

// get occupancy
unsigned char VoxbloxMap::getVoxelState(const Eigen::Vector3d& point) {
  double distance = 0.0;
  if (tsdf_server_->getTsdfMapPtr()->getDistanceAtPosition(point, &distance)) {
    // This means the voxel is observed
...
bool VoxbloxMap::getVoxelCenter(Eigen::Vector3d* center,
  ...
  voxblox::BlockIndex block_id = tsdf_server_->getTsdfMapPtr()
                                     ->getTsdfLayerPtr()
