...
class VoxbloxMap : public TSDFMap {
 public:
  ...
  voxblox::TsdfServer& getTSDFServer();

 protected:
  static ModuleFactoryRegistry::Registration<VoxbloxMap> registration;

  // tsdf server that contains the map, subscribe to external TSDF/TSDF updates
  std::unique_ptr<voxblox::TsdfServer> tsdf_server_;
