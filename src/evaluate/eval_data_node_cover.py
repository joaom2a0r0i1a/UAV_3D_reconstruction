...
class EvalData(object):
    def __init__(self):
        ...
        # Parse parameters
        self.ns_cover = rospy.get_param('~ns_cover',
                                          "/uav1/cover_planner")
        ...
        if self.evaluate:
            ...
            rospy.set_param(self.ns_cover + "/performance_log_dir",
                            self.eval_directory)
            ...
            #    self.ns_cover + "/get_cpu_time", SetBool) # ERROR HEREEEEE
    ...
    def launch_simulation(self):
        ...
        if self.startup_timeout > 0.0:
            try:
                rospy.wait_for_service(self.ns_cover + "/start_waypoints_following",
        ...
        run_planner_srv = rospy.ServiceProxy(
            self.ns_cover + "/start_waypoints_following", Trigger)
        run_planner_srv()
        ...
        if self.reset_ros:
            try:
                ...
                terminate_srv = rospy.ServiceProxy(
                    self.ns_cover + "/stop_waypoints_following", Trigger)
                terminate_srv()
            except:
                pass
        width = len(reason) + 4
