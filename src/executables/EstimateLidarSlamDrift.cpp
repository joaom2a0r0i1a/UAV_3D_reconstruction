...
void printHelp() {
  ...
  ROS_ERROR("Usage:");
  ROS_ERROR(
      "   rosrun pcl_tools estimate_lidar_slam_drift rosbag.bag topic_cloud mapping_origin tf_map_in_target.mat target.pcd trajectory_odom_out.txt "
...
bool parseArguments(int argc, char **argv, ARGUMENTS &args) {
  ...
  if (args.invert_tf_target_in_map_origin) {
    ...
    pcl_tools::printEigenMatrix(tf_target_in_map_origin_inverted.cast<float>(), "Target in map (result of inversion):");

    args.tf_target_in_map_origin = eigenMatrixToTransformMsg(tf_target_in_map_origin_inverted.cast<float>());

  } else {

    pcl_tools::printEigenMatrix(tf_target_in_map_origin.cast<float>(), "Target in map:");
...
std::vector<TRAJECTORY_POINT> estimateGroundTruthTrajectoryFromRosbag(const rosbag::Bag &bag, const PC::Ptr &pc_target, const ARGUMENTS &args) {
  ...
  const PC::Ptr cloud_target = pcl_tools::filters::applyVoxelGridFilter(pc_target, resolution);
  ...
  for (const rosbag::MessageInstance &msg : view) {
    ...
    if (perform_registration) {
      ...
      const PC::Ptr cloud_source = pcl_tools::filters::applyVoxelGridFilter(cloud_buffer.getCloudsAsOne(), resolution);
...
int main(int argc, char **argv) {
  ...
  // Load target cloud
  const auto ret_target = pcl_tools::loadPcXYZ(args.pcd_target);
  if (!ret_target) {
