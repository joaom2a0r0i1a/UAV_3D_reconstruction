...
void printHelp() {
  ...
  ROS_ERROR("Usage:");
  ROS_ERROR("   rosrun pcl_tools pcd_estimate_normals input.pcd output.pcd estimation_radius");
...
int main(int argc, char **argv) {
  ...
  const auto ret_input = pcl_tools::loadPcXYZ(args.pcd_input);
  if (!ret_input) {
  ...
  const PC_NORM::Ptr pc_norm = pcl_tools::estimateNormals(ret_input.value(), args.estimation_radius);

  const sensor_msgs::PointCloud2::Ptr pc_norm_msg = boost::make_shared<sensor_msgs::PointCloud2>();
  pcl::toROSMsg(*pc_norm, *pc_norm_msg);

  pcl_tools::savePCD(args.pcd_output, pc_norm_msg);
