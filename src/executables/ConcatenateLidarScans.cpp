...
void printHelp() {
  ...
  ROS_ERROR("Usage:");
  ROS_ERROR("   rosrun pcl_tools concatenate_lidar_scans rosbag.bag topic_points point_cloud.ply [...]");
...
bool concatenateClouds(PC::Ptr &cloud_out, const rosbag::Bag &bag, const ARGUMENTS &args) {
  ...
  for (const rosbag::MessageInstance &msg : view) {
    ...
    scan = pcl_tools::filters::applyVoxelGridFilter(scan, float(args.voxel_resolution_scans));
    ...
    // Concatenate
    *cloud_out += *scan;
    /* cloud_out = pcl_tools::filters::applyVoxelGridFilter(cloud_out, float(args.voxel_resolution_map)); */
