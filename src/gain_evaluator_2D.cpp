...
GainEvaluator2D::GainEvaluator2D() {}

double GainEvaluator2D::getVerticalFoV(double horizontal_fov, int resolution_x, int resolution_y){
  double aspect_ratio = resolution_x/resolution_y;
  double vertical_fov = 2 * std::atan(std::tan(horizontal_fov / 2) / aspect_ratio);
  return vertical_fov;
}

void GainEvaluator2D::setCameraModelParametersFoV(double horizontal_fov, double vertical_fov,
                                                double min_distance, double max_distance) {
  cam_model_.setIntrinsicsFromFoV(horizontal_fov, vertical_fov, min_distance, max_distance);
}

void GainEvaluator2D::setCameraModelParametersFocalLength(
...
void GainEvaluator2D::setCameraExtrinsics(const voxblox::Transformation& T_C_B) {
  cam_model_.setExtrinsics(T_C_B);
}

void GainEvaluator2D::setTsdfLayer(voxblox::Layer<voxblox::TsdfVoxel>* tsdf_layer) {
...
void GainEvaluator2D::setBounds(float& min_x, float& min_y, float& min_z,
...
void GainEvaluator2D::initialize_neighbour_voxels(float voxel_size) {
  auto vs = voxel_size;
...
/*bool GainEvaluator2D::isFrontierVoxel(const Eigen::Vector3d& voxel) {
...
double GainEvaluator2D::evaluateExplorationGainWithRaycasting(
...
double GainEvaluator2D::computeGain(const eth_mav_msgs::EigenTrajectoryPoint& pose, int modulus) {
...
void GainEvaluator2D::computeGainFromsampledYaw(const std::shared_ptr<rrt_star_2d::Node>& node, int yaw_samples, eth_mav_msgs::EigenTrajectoryPoint& trajectory_point) {
    double best_gain = 0;
    double gain;
    double best_yaw;
...
void GainEvaluator2D::computeCost(std::shared_ptr<rrt_star_2d::Node>& new_node) {
    new_node->cost = new_node->parent->cost + (new_node->point.head(3) - new_node->parent->point.head(3)).norm();
}

void GainEvaluator2D::computeScore(std::shared_ptr<rrt_star_2d::Node>& new_node, double lambda) {
    new_node->score = new_node->parent->score + new_node->gain * exp(-lambda * new_node->cost);
}

double GainEvaluator2D::computeGainInNeighbours(
    const eth_mav_msgs::EigenTrajectoryPoint& pose, int modulus) {
  CHECK_NOTNULL(tsdf_layer_);
...
voxblox::CameraModel& GainEvaluator2D::getCameraModel() { return cam_model_; }

const voxblox::CameraModel& GainEvaluator2D::getCameraModel() const {
  return cam_model_;
}
