...
void PCLPublishCloudFileToNetwork::onInit() {
  ...
  if (_load_colors) {
    ...
    cloud_loaded = pcl_tools::loadCloud(file_cloud, cloud_rgb, true);

  } else {

    const PC::Ptr cloud_xyz = boost::make_shared<PC>();
    cloud_loaded            = pcl_tools::loadCloud(file_cloud, cloud_xyz, true);

    if (cloud_loaded) {
      cloud_rgb = pcl_tools::visualization::colorizeCloud(cloud_xyz);
    }
  }
...
PLUGINLIB_EXPORT_CLASS(pcl_tools::PCLPublishCloudFileToNetwork, nodelet::Nodelet);
