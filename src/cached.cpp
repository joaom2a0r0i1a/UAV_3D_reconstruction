...
public:
    Cached(ros::NodeHandle& nh, const ros::NodeHandle& nh_private) : nh_(nh), nh_private_(nh_private), voxblox_server_(nh_, nh_private_) {
        ...
        std::cout << "resolution x: " << resolution_x << std::endl;
        std::cout << "resolution y: " << resolution_y << std::endl;
        std::cout << "min distance: " << min_distance << std::endl;
        std::cout << "max distance: " << max_distance << std::endl;
        std::cout << "camera frame id: " << camera_frame_id << std::endl;
        std::cout << "body frame id: " << body_frame_id << std::endl;
        std::cout << "frame id: " << frame_id << std::endl;
        std::cout << "gain zero: " << g_zero << std::endl;
...
private:
    ...
    void GetTransformation() {
        T_C_B_message.transform.translation.x = -0.176070;
        T_C_B_message.transform.translation.y = 0.011500;
        T_C_B_message.transform.translation.z = -0.050205;

        T_C_B_message.transform.rotation.x = 0.000000;
        T_C_B_message.transform.rotation.y = -0.382683;
        T_C_B_message.transform.rotation.z = 0.000000;
        T_C_B_message.transform.rotation.w = 0.923880;
    ...
    void timerReevaluate(const ros::TimerEvent&) {
        ...
        hits.clear();
        rtree->Search(bbx_min_range, bbx_max_range, [this, &hits](const cache_nodes::Node& pNode) {
        ...
        for (size_t i = 0; i < reevaluate_list.size(); ++i) {
            ...
            rtree->Remove(bbx_min_range, bbx_max_range, reevaluate_list[i]);
            rtree->Insert(bbx_min_range, bbx_max_range, reevaluate_list[i]);
        }
    ...
    bool callbackBestNode(cache_nodes::BestNode::Request& req, cache_nodes::BestNode::Response& res) {
        ...
        rtree->Search(bbx_min, bbx_max, [this, &hits_best](const cache_nodes::Node& pNode) {
