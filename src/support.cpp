...
void publishCloud(const ros::Publisher &pub, const pcl::PointCloud<T> &cloud) {
  if (pub.getNumSubscribers() > 0) {
    try {
      ...
      ROS_ERROR("[pcl_tools::publishCloud]: Exception caught during publishing on topic: %s", pub.getTopic().c_str());
    }
  }
}
/*//}*/
...
void publishCloudMsg(const ros::Publisher &pub, const sensor_msgs::PointCloud2::ConstPtr &cloud_msg) {
  if (pub.getNumSubscribers() > 0) {
    try {
      ...
      ROS_ERROR("[pcl_tools::publishCloudMsg]: Exception caught during publishing on topic: %s", pub.getTopic().c_str());
    }
  }
}
/*//}*/
