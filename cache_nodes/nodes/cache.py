...
from cache_nodes.msg import Node
from cache_nodes.srv import Query, QueryResponse
from cache_nodes.srv import BestNode, BestNodeResponse
from cache_nodes.srv import Reevaluate
...
class Cache:
    def __init__(self):
        self.ss_query = rospy.Service('~gp_query_in', Query, self.callbackQuery)
        self.ss_best_node = rospy.Service('~best_node_in', BestNode, self.callbackBestNode)
        self.sc_reevaluate = rospy.ServiceProxy('~reevaluate_out', Reevaluate)
        ...
        self.g_zero          = rospy.get_param('~local_planning/g_zero',  2)

        self.sub_gain = rospy.Subscriber('~tree_node_in', Node, self.callbackGain)
        self.sub_uav_state = rospy.Subscriber('~uav_state_in', UavState, self.callbackUavState)
        self.pub_marker = rospy.Publisher('~gain_markers', MarkerArray, queue_size=10)
        self.pub_mean = rospy.Publisher('~mean_markers', MarkerArray, queue_size=10)
        self.pub_sigma = rospy.Publisher('~sigma_markers', MarkerArray, queue_size=10)
        ...
        rospy.logwarn("Defaulting to (-25, -25, 0.5), (25,  25, 15)...")
        self.min = [-25, -25, 0.5]
        self.max = [25,  25, 15]
        
        # Get Sensor Range (2*range)
        rospy.logwarn("Range max parameter not specified")
        rospy.logwarn("Defaulting to 10 m...")
        self.range = 10
        ...
        #rospy.Timer(rospy.Duration(5), self.timerReevaluate)
    ...
    '''def timerReevaluate(self, event):
    ...
    def callbackQuery(self, req):
        ...
        rospy.loginfo("QUERY ADD TO DATA")
        ...
        rospy.loginfo("NEAREST OBJECT")
        yaw = 0
        for item in nearest:
            yaw = item.object.yaw

        rospy.loginfo("QUERY EMPTY")
        if y.shape[0] == 0:
        ...
        rospy.loginfo("QUERY END")

        return response

    """ Return all nodes with gain higher than req.threshold """
    def callbackBestNode(self, req):
    ...
    def evaluate(self, event):
        ...
        self.pub_mean.publish(mean_markers)
        self.pub_sigma.publish(sigma_markers)
